...
export default function StitchConverter({ result }: StitchConverterProps) {
  ...
  const gaugeRatio = hasResult ? result.estimatedGauge / result.patternGauge : null;
  const rowRatio =
    result?.estimatedRowGauge !== undefined && result?.patternRowGauge !== undefined
      ? result.estimatedRowGauge / result.patternRowGauge
      : null;

  const stitchVal = parseFloat(patternStitchCount);
  const newStitchCount =
    gaugeRatio !== null && patternStitchCount !== "" && !isNaN(stitchVal) && stitchVal > 0
    ...
    <div className={styles.converter}>
      {!hasResult && (
        <p className={styles.emptyHint}>
          Estimate your gauge above first — then enter your pattern&rsquo;s stitch counts here to convert them.
        </p>
      )}
      ...
      {newStitchCount !== null && (
        <div className={styles.result}>
          <div className={styles.resultItem}>
            <span className={styles.resultLabel}>Cast on</span>
